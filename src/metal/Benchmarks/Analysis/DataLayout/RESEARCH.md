...
| 配置 | AoS | SoA | Hybrid | 说明 |
|------|-----|-----|--------|------|
| 1024粒子 | 0.3 M/s | 0.2 M/s | 0.2 M/s | 相似性能 |
| 4096粒子 | 0.8 M/s | 0.8 M/s | 0.8 M/s | 线性扩展 |
| 16384粒子 | 2.9 M/s | 2.8 M/s | 2.9 M/s | 带宽饱和 |

**重要发现**: Apple M2上AoS和SoA性能相近，与传统认知不同！
...
### 意外发现：AoS vs SoA 性能相近

在传统离散GPU上，AoS因跨步访问会有2-4x的性能损失。但在Apple M2上实测显示：

| 指标 | 传统GPU | Apple M2 |
|------|---------|----------|
| AoS vs SoA | 2-4x慢 | ~1x (相近) |
| 原因 | 独立显存 | 统一内存共享 |
| 缓存效率 | SoA更优 | 两者相近 |

### 可能原因分析

1. **统一内存架构**: CPU和GPU共享内存，内存控制器可能统一优化了访问模式
2. **更大的缓存行**: Apple M2可能使用更大的内部缓存行
3. **硬件预取**: 智能预取可能隐藏了AoS的跨步访问延迟
4. **编译器优化**: Metal编译器可能自动优化了AoS访问模式

### 实际建议

尽管Apple M2上AoS和SoA性能相近，**仍然推荐使用SoA**，因为：

1. **可移植性**: 代码在其他GPU上仍有效
2. **代码清晰度**: 字段级操作更直观
3. **向量化便利**: SoA更容易使用float4向量化
4. **未来-proof**: 统一内存是Apple特有架构
