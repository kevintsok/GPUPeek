...
// =============================================================================
// TCGen05 Instruction Examples (from CUTLASS mma_sm100_umma.hpp)
// =============================================================================
//
// Example 1: TF32 SS (SMEM-SMEM) - Both operands from SMEM
// ------------------------------------------------------------------
// struct SM100_MMA_TF32_SS {
//   void fma(uint64_t const& desc_a,   // A matrix descriptor (SMEM)
//             uint64_t const& desc_b,   // B matrix descriptor (SMEM)
//             uint32_t const& tmem_c,   // Accumulator in TMEM
...
// };
//
// Example 2: FP16 TS (TMEM-SMEM) - A from TMEM, B from SMEM
// ------------------------------------------------------------------
// struct SM100_MMA_F16BF16_TS {
//   void fma(uint32_t const& tmem_a,  // A from TMEM
//             uint64_t const& desc_b,   // B from SMEM
//             uint32_t const& tmem_c,
//             ...);
// };
//
// Example 3: Block Scaled MMA
// ------------------------------------------------------------------
// struct SM100_MMA_TF32_SS_SCALED {
//   void fma(..., uint32_t ScaleC) {  // ScaleC as immediate
//     asm volatile(
//       "tcgen05.mma.cta_group::1.kind::tf32 [...], p, %9;"
//       : : ... "n"(ScaleC));  // ScaleC as immediate
//   }
// };

// =============================================================================
// SM120 Native MMA (16x8x32) - Register-based FP4/FP6
// =============================================================================
//
// These are the "simple" MMA instructions available on SM120
...
// =============================================================================
// How to Use CUTLASS for TCGen05
// =============================================================================
//
// Instead of writing raw TCGen05 kernels, use CUTLASS:
//
// 1. Include CUTLASS headers:
//    #include "cutlass/gemm/device/gemm.h"
//    #include "cutlass/gemm/collective/collective_builder.hpp"
//
// 2. Define types (from 79a_blackwell_geforce_nvfp4_bf16_gemm.cu):
//    using ElementA = cutlass::nv_float4_t<cutlass::float_e2m1_t>;  // FP4
//    using ElementB = cutlass::bfloat16_t;
//    using ElementD = cutlass::bfloat16_t;
//
// 3. Use CollectiveBuilder:
...
//    >::CollectiveOp;
//
// 4. Build and run via device::GemmUniversalAdapter
